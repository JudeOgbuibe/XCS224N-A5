...
def pad_sents_char(sents, char_pad_token):
    ...
    for s in sents:
        ...
        for w in s:
            ...
            #w_padded[:len(w)] = w
            #ws_padded.append(w_padded)
            if len(w) > max_word_length:
                ws_padded.append(w[:max_word_length])
            else:
                padding = max_word_length - len(w)
                ws_padded.append(w + [char_pad_token] * padding)

        padded = [[char_pad_token] * max_word_length] * max_len
        padded[:len(ws_padded)] = ws_padded 
